...
template <typename T>
void PFHShapeReco<T>::ComputePFH(CloudPtr& cloudPtr, pcl::PointCloud<pcl::Normal>::Ptr& normalsCloudPtr)
{
  _cloudPtr = cloudPtr;
...
template <typename T>
std::vector<double>& PFHShapeReco<T>::GetObjectModel()
{
  return this->_objectModel;
}

template <typename T>
void PFHShapeReco<T>::SetObjectModel(const std::vector<double> model)
{
...
void PFHShapeReco<T>::IdentifyObject()
{
  ...
  for (typename std::vector<sModel>::const_iterator modelsIt = _modelsDatabase.begin(); modelsIt != _modelsDatabase.end(); ++modelsIt)
  {
    double dist = 0.0;
    for(size_t j = 0; j < _sizeHist; j++)
    {
      // Make sure no value is 0 (because we divide afterwards)
      double valObj = std::max(std::numeric_limits<double>::min(), _objectModel[j]);
      double valMod = std::max(std::numeric_limits<double>::min(), modelsIt->modelHist[j]);

      // Compute Kullback-Leibler distance
      dist += (valObj-valMod)*log(valObj/valMod);
    }
    // Save minimum distance for each model name
    std::map<std::string,double>::iterator scoreIt = _identificationScores.find(modelsIt->modelName);
...
template <typename T>
std::map<std::string,double> PFHShapeReco<T>::GetIdentificationScores()
{
