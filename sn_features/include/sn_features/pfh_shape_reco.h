...
public:
  ...
  void ComputePFH(CloudPtr& cloudPtr, pcl::PointCloud<pcl::Normal>::Ptr& normalsCloudPtr);
  ...
  std::map<std::string,double> GetIdentificationScores();
  // Get histogram of the current object
  std::vector<double>& GetObjectModel();
...
protected:
  ...
  void SetObjectModel(const std::vector<double> model);

private:
  // Histogram of the current object
  std::vector<double> _objectModel;
  ...
  double _fAngleFactor, _fDistFactor;
  ...
  std::map<std::string,double> _identificationScores;
};
